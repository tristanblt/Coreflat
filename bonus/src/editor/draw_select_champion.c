...
bool disasm_champion(cw_graph_t *cw_graph, champion_t **champions)
{
    ...
    for (int i = 0; i < cw_graph->edit.header->prog_size;) {
        if (!(to_add = parse_instruction_from_file(champions[cw_graph->edit.selected]->instructions, &i,
...
bool draw_select_champion(cw_graph_t *cw_graph, champion_t **champions)
{
    ...
    draw_button(cw_graph, (sfVector2f) {800, 780}, cw_graph->interface_gradient, VALIDATE_CHAMPION);
    return (true);
}