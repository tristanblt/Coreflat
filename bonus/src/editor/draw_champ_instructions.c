...
instruction_t *instruction, int j)
{
    ...
    cw_graph->edit.number_selected) {
        ...
        instruction->description[j] = cw_graph->edit.indirect_type ?
    T_IND : T_DIR;
    }
    ...
    draw_rect(cw_graph, (sfVector2f) {220 + dec * 20 + j * 80, 170 + i * 39.3 +
    ...
    if (instruction->label_args[j]) {
        ...
        (sfVector2f) {(instruction->description[j] == T_DIR ? 235 : 220) +
            dec * 20 + j * 80, 170 + i * 39.3 + cw_graph->edit.cursor});
        return;
    }
    if (instruction->description[j] == T_IND)
        draw_text(cw_graph, int_to_str(instruction->args[j]), 16,
        (sfVector2f) {220 + dec * 20 + j * 80, 170 + i * 39.3 +
    ...
    if (instruction->description[j] == T_DIR) {
        ...
        (sfVector2f) {220 + dec * 20 + j * 80, 170 + i * 39.3 +
            cw_graph->edit.cursor});
        draw_text(cw_graph, int_to_str(instruction->args[j]), 16,
        (sfVector2f) {240 + dec * 20 + j * 80, 170 + i * 39.3 +
            cw_graph->edit.cursor});
    }
    if (instruction->description[j] == T_REG) {
        ...
        (sfVector2f) {220 + dec * 20 + j * 80, 170 + i * 39.3 +
            cw_graph->edit.cursor});
        draw_text(cw_graph, int_to_str(instruction->args[j]), 16,
        (sfVector2f) {230 + dec * 20 + j * 80, 170 + i * 39.3 +
            cw_graph->edit.cursor});
    }
}
